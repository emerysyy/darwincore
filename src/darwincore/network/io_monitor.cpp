...
IOMonitor::IOMonitor()
    : monitor_fd_(-1), timeout_ms_(1000) {
...
bool IOMonitor::Initialize() {
  ...
  NW_LOG_INFO("[IOMonitor::Initialize] IO 监控器初始化成功，fd=" << monitor_fd_);
  return true;
}
...
bool IOMonitor::StartReadMonitor(int fd) {
  ...
  if (ret < 0) {
    NW_LOG_ERROR("[IOMonitor::StartReadMonitor] 添加监控 fd=" << fd << " 失败: "
              << strerror(errno));
    return false;
  }
...
bool IOMonitor::StopMonitor(int fd) {
  ...
  if (ret < 0) {
    ...
    if (errno != ENOENT) {
      NW_LOG_WARNING("[IOMonitor::StopMonitor] 停止监控 fd=" << fd << " 失败: "
                << strerror(errno));
    }
    return false;
  }
...
#if USE_KQUEUE
  int count = kevent(monitor_fd_, nullptr, 0,
                   static_cast<struct kevent*>(events), max_events, timeout_ptr);
#elif USE_EPOLL
