...
private:
  ...
  std::vector<std::unique_ptr<Acceptor>> acceptors_;  ///< 多个 Acceptor 支持双栈监听
  std::unordered_map<uint64_t, size_t> conn_to_reactor_;  ///< connection_id -> reactor_index
...
bool Server::Impl::InitializeWorkerPool() {
  if (worker_pool_ == nullptr) {
    worker_pool_ = std::make_shared<WorkerPool>(SocketConfiguration::kDefaultWorkerCount);
    if (!worker_pool_->Start()) {
      NW_LOG_ERROR("[Server::InitializeWorkerPool] WorkerPool 启动失败");
      return false;
    }
    NW_LOG_INFO("[Server] WorkerPool 已启动，worker_count=" << SocketConfiguration::kDefaultWorkerCount);
  }
  return true;
}
...
bool Server::Impl::InitializeReactors() {
  ...
  // Reactor 数量 = CPU 核数
  size_t reactor_count = std::max(1, static_cast<int>(std::thread::hardware_concurrency()));
...
bool Server::Impl::SendData(uint64_t connection_id, const uint8_t* data,
  ...
  // connection_id = (reactor_id << 32) | local_id
  size_t reactor_index = static_cast<size_t>(connection_id >> 32);
  if (reactor_index >= reactors_.size()) {
    NW_LOG_ERROR("[Server::SendData] reactor_index=" << reactor_index
              << " 超出范围 (size=" << reactors_.size() << ")");
    return false;
  }
  return reactors_[reactor_index]->SendData(connection_id, data, size);
}

void Server::Impl::SetOnClientConnected(Server::OnClientConnectedCallback callback) {
...
void Server::Impl::SetOnClientDisconnected(Server::OnClientDisconnectedCallback callback) {
  on_client_disconnected_ = std::move(callback);
}

void Server::Impl::SetOnConnectionError(Server::OnConnectionErrorCallback callback) {
  on_connection_error_ = std::move(callback);
}

void Server::Impl::OnNetworkEvent(const NetworkEvent& event) {
  NW_LOG_TRACE("[Server::Impl::OnNetworkEvent] type=" << static_cast<int>(event.type)
            << ", conn_id=" << event.connection_id
  ...
  switch (event.type) {
    ...
    case NetworkEventType::kData:
      NW_LOG_TRACE("[Server] kData 事件, on_message_=" << (on_message_ != nullptr));
      if (on_message_) {
    ...
    case NetworkEventType::kError:
      if (on_connection_error_ && event.error) {
        on_connection_error_(event.connection_id, *event.error, event.error_message);
      }
      break;
  }
}
