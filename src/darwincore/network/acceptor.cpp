...
Acceptor::Acceptor()
    : listen_fd_(-1), io_monitor_(nullptr), is_running_(false), next_reactor_index_(0) {
...
bool Acceptor::ListenGeneric(SocketProtocol protocol,
  ...
  if (bind(fd, reinterpret_cast<const sockaddr*>(&addr), addr_len) < 0) {
    NW_LOG_ERROR("[Acceptor::ListenGeneric] bind 失败: " << strerror(errno) << ", fd=" << fd);
    close(fd);
    return false;
  }
...
void Acceptor::AcceptLoop() {
    ...
    for (int i = 0; i < nev; ++i) {
      ...
      if (fd == listen_fd_) {
        ...
        if (client_fd < 0) {
          ...
          NW_LOG_ERROR("[Acceptor::AcceptLoop] accept 失败: " << strerror(errno));
          // 其他错误，退出循环
          break;
        }

        NW_LOG_TRACE("[Acceptor::AcceptLoop] 接受新连接，client_fd=" << client_fd);

        // 设置客户端 Socket 为非阻塞
        if (!SocketHelper::SetNonBlocking(client_fd)) {
          NW_LOG_ERROR("[Acceptor::AcceptLoop] 设置非阻塞失败，关闭 client_fd=" << client_fd);
          close(client_fd);
          continue;
        }

        NW_LOG_DEBUG("[Acceptor::AcceptLoop] 新连接 fd=" << client_fd << " 准备分配给 Reactor");
        // 将新连接分配给 Reactor
  ...
  // 停止监听 listen_fd
  NW_LOG_DEBUG("[Acceptor::AcceptLoop] AcceptLoop 结束，停止监听 listen_fd=" << listen_fd_);
...
void Acceptor::AssignToReactor(int fd, const sockaddr_storage& peer) {
  ...
  NW_LOG_DEBUG("[Acceptor] reactors_.size=" << reactors_.size() << ", 选择的 reactor_index=" << index);
  ...
  NW_LOG_DEBUG("[Acceptor] 将 fd=" << fd << " 添加到 Reactor (reactor_id=" << reactor->GetReactorId() << ")");
  // 将 fd 添加到 Reactor（Reactor 内部会提交 kConnected 事件）
  uint64_t connection_id = reactor->AddConnection(fd, peer);
  if (connection_id == 0) {
    NW_LOG_ERROR("[Acceptor::AssignToReactor] Reactor::AddConnection 失败，关闭 fd=" << fd);
    close(fd);
  } else {
    NW_LOG_DEBUG("[Acceptor] Reactor::AddConnection 成功，conn_id=" << connection_id);
  }
}
